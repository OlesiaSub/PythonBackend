...
from expense_validation.validator import validate_id, validate_expense, validate_group
from model.tracker_model import Expense, Group


def process_get_expense_by_id(expense_id: int, db: Session):
    id_int = int(expense_id)
    validate_id(id_int)
    db_expense = db_queries.get_expense_by_id(db, expense_id=id_int)
    if db_expense is None:
        raise HTTPException(status_code=404, detail="This expense ID is not present in the database")
    return db_expense
